    ...
    <template v-else>
      <div class="panel">
        <p class="panel-heading">
          {{$t('trusted-sources')}}
          <button class="button is-link" @click="startSourceCreation()" v-if="!currentUser.guestByNow">
      ...
      >
        ...
        <template #default="{row: source}">
          ...
          <b-table-column label="" width="50">
            <div class="buttons" v-if="canEdit(source)">
...
import TrustedSourceModal from '@/components/admin/TrustedSourceModal';
import {getWildcardRegexp} from '@/utils/string-utils';
import SoftwareSource from '@/components/software/SoftwareSource';
import {get} from '@/utils/store-helpers';

export default {
  name: 'list-trusted-sources',
  components: {
    SoftwareSource,
    TrustedSourceModal,
  },
  ...
  methods: {
    ...
    canEdit(source) {
      return this.currentUser.adminByNow || this.currentUser.id === source.user;
    }
  },
  async created() {
    try {
